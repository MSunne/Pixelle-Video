...
Generates videos from images and text prompts using ComfyUI workflows.
Extracted from web/pipelines/i2v.py as a standalone, UI-independent service.

Example:
    pipeline = I2VPipeline(pixelle_video)
    result = await pipeline(
        image_paths=["/path/to/image.jpg"],
        prompt="A girl is dancing in the rain",
        workflow_key="runninghub/i2v_xxx.json",
    )
"""
...
@dataclass
class I2VResult:
    """Result of image-to-video generation."""
    video_path: str = ""
    file_size: int = 0
    task_id: str = ""
    task_dir: str = ""
...
class I2VPipeline:
    """
    Image-to-Video Pipeline (UI-independent).
    
    Generates videos from images and prompts using ComfyUI workflows.
    """
    ...
    async def __call__(
        ...
        Args:
            image_paths: List of image paths (at least one)
            prompt: Text prompt for video generation
            workflow_key: Workflow key (e.g. "runninghub/i2v_xxx.json")
            progress_callback: Optional progress callback
            
        Returns:
            I2VResult with video path, file size
        """
        ...
        try:
            ...
            logger.error(f"[I2V] Pipeline failed: {e}")
            raise
    ...
    async def _download_video(self, video_url: str, output_path: str):
        ...
        logger.info(f"[I2V] Video downloaded: {output_path}")
